...
function Pelanggan() {
  //title page
  document.title = "Pelanggan - Toko Ku";
  ...
  const fetchData = async (pageNumber, searchData) => {
    ...
    setLoading(true);
    await Api.get(`/api/pelanggan/list?search=${searchQuery}&page=${page}`, {
      headers: {
        //header Bearer + Token
      ...
      setPeLanggan(response.data.data.data);
      ...
      buttons: [
        {
          label: "Ya Hapus",
          onClick: async () => {
            await Api.delete(`/api/pelanggan/delete/${id}`, {
              headers: {
                //header Bearer + Token
                Authorization: `Bearer ${token}`,
              },
            ...
            <div className="card border-0 rounded shadow-sm border-top-success">
              <div className="card-header">
                <span className="font-weight-bold">
                  <i className="fa fa-users"></i> PELANGGAN
                </span>
              </div>
                      ...
                      {loading ? (
                        ...
                        pelanggan.map((pel, index) => (
                          <tr key={index}>
                            ...
                            <td className="text-center">{pel.nama}</td>
                            <td className="text-center">{pel.domisili}</td>
                            <td className="text-center">{pel.jenis_kelamin}</td>
                            <td className="text-center">
                              <Link
                                to={`/pelanggan/ubah/${pel.id_pelanggan}`}
                              ...
                              <button
                                onClick={() =>
                                  deletePelanggan(pel.id_pelanggan)
                                }
